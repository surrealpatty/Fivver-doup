...
const initModels = () => {
    ...
    if (User && User.associate) User.associate({ Service, UserProfile });
    if (Service && Service.associate) Service.associate({ User });
    if (UserProfile && UserProfile.associate) UserProfile.associate({ User });
};
