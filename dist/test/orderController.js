...
const mockUser = {
    ...
    username: 'testuser',
    role: 'user', // Explicitly assert the value is of type UserRole
    tier: 'free', // Explicitly assert the value is of type UserTier
    isVerified: false,
};
describe('User Registration Controller', () => {
    it('should register a user successfully', async () => {
        // Mock the User.create method to return the mock user
        jest.spyOn(user_1.default, 'create').mockResolvedValue({
            ...mockUser,
    ...
    it('should handle user registration failure due to existing email', async () => {
        ...
        jest.spyOn(user_1.default, 'findOne').mockResolvedValue(mockUser);
        // Mock request and response objects
        const req = {
            body: {
                email: 'test@example.com',
    ...
    it('should handle internal server errors gracefully', async () => {
        ...
        jest.spyOn(user_1.default, 'create').mockRejectedValue(new Error('Database error'));
        // Mock request and response objects
        const req = {
            body: {
                email: 'test@example.com',
