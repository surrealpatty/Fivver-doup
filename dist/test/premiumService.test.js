...
const types_1 = require("../types"); // Import UserTier and UserRole enums for correct tier and role values
...
const mockPaidUser = {
    ...
    username: 'paiduser',
    tier: types_1.UserTier.Paid, // Use UserTier.Paid instead of 'paid'
    role: types_1.UserRole.User, // Add the role property
};
const mockFreeUser = {
    id: '2',
    email: 'freeuser@example.com',
    username: 'freeuser',
    tier: types_1.UserTier.Free, // Use UserTier.Free instead of 'free'
    role: types_1.UserRole.User, // Add the role property
};
...
jest.mock('jsonwebtoken', () => ({
    ...
    verify: jest.fn().mockImplementation((token, secret) => {
        ...
        if (token === 'mocked-token-1') {
            return { id: '1', username: 'paiduser', tier: types_1.UserTier.Paid, role: types_1.UserRole.User }; // Ensure role and tier are valid
        }
        if (token === 'mocked-token-2') {
            return { id: '2', username: 'freeuser', tier: types_1.UserTier.Free, role: types_1.UserRole.User }; // Ensure role and tier are valid
        }
