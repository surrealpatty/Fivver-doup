...
describe('Role-based Access Tests', ()=>{
    ...
    beforeAll(async ()=>{
        ...
        const response = await (0, _supertest.default)(_index.app).post('/api/register') // Adjust to your actual registration route
        .send({
            username: 'testuser',
    ...
    afterAll(async ()=>{
        ...
        if (testUser && testUser.id) {
            await (0, _supertest.default)(_index.app).delete(`/api/users/${testUser.id}`) // Adjust to your actual user deletion route
    ...
    it('should deny access to premium service for free users', async ()=>{
        const response = await (0, _supertest.default)(_index.app).get('/api/premium-service') // Adjust to your actual premium service route
    ...
    it('should allow access to premium service for paid users', async ()=>{
        ...
        const paidUserResponse = await (0, _supertest.default)(_index.app).post('/api/update-role') // Adjust to your actual role update route
        .send({
            userId: testUser.id,
            role: 'paid'
