...
describe('Role-based Access Tests', ()=>{
    ...
    beforeAll(async ()=>{
        ...
        const response = await (0, _supertest.default)(_index.default).post('/register') // Replace with your registration route
        .send({
            username: 'testuser',
    ...
    afterAll(async ()=>{
        ...
        if (testUser && testUser.id) {
            await (0, _supertest.default)(_index.default).delete(`/users/${testUser.id}`); // Example cleanup, adjust as needed
        }
    });
    ...
    it('should deny access to premium service for free users', async ()=>{
        const response = await (0, _supertest.default)(_index.default).get('/premium-service') // Assuming your endpoint for premium access
    ...
    it('should allow access to premium service for paid users', async ()=>{
        ...
        const paidUserResponse = await (0, _supertest.default)(_index.default).post('/update-role') // Assuming you have an endpoint for updating role
        .send({
            userId: testUser.id,
            role: 'paid'
        });
