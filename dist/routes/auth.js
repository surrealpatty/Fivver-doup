...
router.post('/register', [
    ...
    try {
        ...
        const newUser = yield User.create({ email, password: hashedPassword });
...
router.get('/profile', authenticateToken, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const user = yield User.findByPk(req.user.id); // Ensure req.user is set by authenticateToken middleware
        if (!user) {
            return res.status(404).json({ message: 'User not found' });
        }
