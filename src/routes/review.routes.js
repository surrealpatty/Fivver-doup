...
import { createService, getServices, updateService, deleteService, } from '../controllers/serviceController'; // Ensure correct path and named exports
const router = Router();
...
router.put('/:id', authMiddleware, authorizeRoles('user', 'admin'), async (req, res) => {
    try {
        // Ensure req and res are passed to the controller functions
...
router.delete('/:id', authMiddleware, authorizeRoles('user', 'admin'), async (req, res) => {
    try {
        // Ensure req and res are passed to the controller functions
